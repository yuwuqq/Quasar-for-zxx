...
namespace Quasar.Common.Cryptography
{
    public class Aes256
    {
        ...
        public byte[] Encrypt(byte[] input)
        {
            ...
            {
                ms.Position = HmacSha256Length; // 为HMAC保留前32字节
                using (var aesProvider = new AesCryptoServiceProvider())
                {
                    ...
                    {
                        ms.Write(aesProvider.IV, 0, aesProvider.IV.Length); // 写下16个字节的IV，然后是密码文本
        ...
        public byte[] Decrypt(byte[] input)
        {
            ...
            using (var ms = new MemoryStream(input))
            {
                using (var aesProvider = new AesCryptoServiceProvider())
                {
                    ...
                    ms.Read(iv, 0, IvLength); // 读取下一个16字节的IV，然后是密码文本
                    aesProvider.IV = iv;
